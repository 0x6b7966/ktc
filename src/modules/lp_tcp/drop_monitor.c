...
static int tcp_dm_cmd_config(struct sk_buff *skb,
			struct genl_info *info)
{
	return -ENOTSUPP;
}

static int tcp_dm_cmd_trace(struct sk_buff *skb,
			struct genl_info *info)
{
	switch (info->genlhdr->cmd) {
	case TCP_DM_CMD_START:
		pr_warn("start");
		return set_all_monitor_traces(TRACE_ON);
		break;
	case TCP_DM_CMD_STOP:
		pr_warn("stop");
...
static const struct genl_ops dropmon_ops[] = {
	{
		.cmd = TCP_DM_CMD_CONFIG,
		.doit = tcp_dm_cmd_config,
	},
	{
		.cmd = TCP_DM_CMD_START,
		.doit = tcp_dm_cmd_trace,
	},
	{
		.cmd = TCP_DM_CMD_STOP,
		.doit = tcp_dm_cmd_trace,
...
static int init_tcp_drop_monitor(void)
{
	...
	pr_warn("after register_netdevice_notifier_rh");
	rc = 0;
...
out_unreg:
	genl_unregister_family(&tcp_drop_monitor_family);
out:
	return rc;
}
